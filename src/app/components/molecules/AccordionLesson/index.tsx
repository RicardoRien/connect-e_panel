...
export function AccordionLesson(props: Props) {
  ...
  return (
    <Accordion
      sx={{ backgroundColor: accordionColor, borderRadius: '8px!important', marginY: '15px' }}
      onClick={handleClick}
    >
      <AccordionSummary expandIcon={<ExpandMoreIcon style={{ color: expandIconColor }} />}>
        <Box sx={{
          backgroundColor: badgeColor, padding: '10px', minWidth: '100px', textAlign: 'center',
          borderRadius: '70px', height: '08px'
        }}>
          <Typography color={badgeTextColor} sx={{ position: 'relative', bottom: '7px', fontWeight: '600' }}>
            {badgeText}
          </Typography>
        </Box>
      </AccordionSummary>
      <Box sx={{ paddingLeft: '20px', paddingBottom: '10px' }}>
        <Typography sx={{ color: textColor, fontWeight: '900' }}>{props.title}</Typography>
